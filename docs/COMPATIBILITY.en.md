...
Each Probe Job clones the Catalog-pinned upstream commit first, then detects Python and GCC capability from that source's `include/prereq-build.mk`; Source/Branch names must not select the runtime. A compatibility runtime is installed only when the detected contract requires it, then upstream `make prepare-mk` validates host prerequisites without depending on a Target/Profile `.config` that does not exist yet. Evidence records direct Root count, package count after Defconfig, Python identity, and compiler identity separately.
