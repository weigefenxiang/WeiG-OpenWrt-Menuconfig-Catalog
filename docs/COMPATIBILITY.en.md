...
- **L1 Plugin (`config-resolve`)**: by default use the current source's official Kconfig/Defconfig resolver for the Final Root combination. When the optional A/B comparison is enabled, solve the exclusion Baseline B first and Final A second, retaining one paired record per environment. This proves configuration viability only and does not compile packages.
