...
The browser compresses the resolved Advanced menuconfig `PACKAGE_*` state plus probe controls and pre-fills the dedicated GitHub Issue form; no separate request file is generated or uploaded. The Issue gateway validates the state token, SHA-256, Issue identity, and actor permission, dispatches the matching code channel, and the worker re-reads the same Issue state and verifies its hash.
