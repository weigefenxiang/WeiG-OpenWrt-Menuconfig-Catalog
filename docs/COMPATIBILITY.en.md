...
L2-L7 execute Final only and reuse completed Stages; they do not build comparison firmware or repeat builds merely to increase depth. Success means the Final configuration reached the selected depth in that environment. Failure describes that Final configuration and Stage without assigning independent causality to one plugin. L1 always uses the official resolver; L2-L7 default Defconfig on but allow an explicit request to disable it.
