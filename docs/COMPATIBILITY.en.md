...
- **L1 Plugin (`config-resolve`)**: by default use the current source's official Kconfig/Defconfig resolver for the Final Root combination. When the optional A/B comparison is enabled, first preflight the direct plugin Root against metadata from the installed Feeds for the current Source/Branch, then solve the exclusion Baseline B and Final A, retaining one paired record per environment. If the Root is absent from that source/branch, record `skipped/root-absent-source`; both B and A are `not-run`, with no Target or compile stage. This proves configuration viability only and does not compile packages.
...
Execution status and compatibility conclusion are separate axes. `target-prerequisite-failure`, `package-compile-prerequisite-failure`, `rootfs-*-prerequisite-failure`, and `firmware-prerequisite-failure` may be reported when strong deterministic upstream prerequisite evidence exists, with only `patch-apply`, `toolchain-kernel-version`, `kernel-prerequisite`, and `target-build` allowed as generic causes. The result remains `inconclusive` (reported inconclusive), but the environment Job may finish successfully. Evidence must retain the innermost failed target and deterministic error summary, for example `package/kernel/linux`, `kernel-prerequisite`, and `module .../crypto/geniv.ko is missing`; these causes do not increase the plugin-caused rate. A paired Baseline-B wrapper may be green only when its inner phase has one of these allowlisted causes. Network, downloads, disk, OOM/Killed, Runner, timeout, missing metadata/evidence, and unattributed failures remain operational/unattributed inconclusive and must stay red. Final compatibility conclusions read normalized evidence only; Job green/red is never a substitute for the evidence conclusion.
