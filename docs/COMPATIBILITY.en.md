...
The browser downloads the schema-1 request as a public, auditable `probe-request.json` and opens the dedicated GitHub Issue form; the user only drags that JSON into the required upload field. A lightweight Issue gateway on the default branch validates the single attachment, UTF-8/JSON/schema, SHA-256, Issue identity, and actor permission before dispatching the execution workflow to the matching `dev`, `staging`, or `main` code channel in the request. The worker downloads the same attachment again and verifies its hash, so any post-dispatch change is rejected. The gateway is event-driven, has no polling or six-hour idle schedule, and must itself be promoted safely to the default branch before independent channel execution is available.

Code `main` and production data `catalog-data` have independent lifecycles. Builder `main` writes only `catalog-candidate`, while runtime/probe `main` continues to read `catalog-data`. The production branch has exactly one writer: the manually dispatched Production Gate. It verifies the candidate's `main` code SHA, completeness, and every indexed asset contract, then promotes that exact snapshot without rebuilding. Scheduled translation writes only the candidate and scheduled curated-size refreshes write only `dev`, so ordinary pushes, schedules, and probe experiments cannot bypass the Production Gate.

The repository owner may use the full plan concurrency, write collaborators are capped at three, and ordinary visitors may inspect the UI and public Runs but cannot start the Matrix. The requester or a write/maintain/admin collaborator can reply with exactly `/cancel` in the same Issue. Normal cancellation runs first and force cancellation is used only while the Run remains active; repeated cancellation, queued cancellation, and dispatch races converge idempotently on the Issue identity. `workflow_dispatch` remains the administrator fallback. Normalized evidence is retained for 60 days and complete logs for 30 days. Plan-only output explicitly means no compilation ran and no compatibility conclusion exists. Cancellation, timeout, and infrastructure failures remain `inconclusive`; the Summary job does not turn them into a second package failure.

When several packages fail together, the Runner performs generic delta reduction only after every planned target shows a package-stage failure, bounded by the per-mode budget in `.github/automation-policy.json`. The output is a “bounded reduction candidate,” never an automatic rule. Dependency installation, exact cloning, feeds, builds, and boot output are combined in the complete probe log. Expensive tools/toolchain, package, RootFS, and firmware targets first run with the Runner's available CPU count plus one; a failure is then reproduced with `-j1 V=s`. Configuration and cleanup targets such as `defconfig`, `clean`, and `dirclean` remain serial. A parallel failure recovered by the serial retry is evidence of recovery, not incompatibility.

A GitHub-hosted job cannot run without a platform limit. All four probe depths use the maximum 360-minute job duration declared by `.github/automation-policy.json`. Reaching that hard limit or another timeout yields only `inconclusive`, never package-failure evidence.
