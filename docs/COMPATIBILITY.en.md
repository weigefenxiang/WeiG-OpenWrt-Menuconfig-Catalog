...
Each Probe Job clones the Catalog-pinned upstream commit first, then detects Python and GCC capability from that source's `include/prereq-build.mk`; Source/Branch names must not select the runtime. A compatibility runtime is installed only when the detected contract requires it, and upstream `make prereq` must pass before feeds or Probe execution begin. Evidence records direct Root count, package count after Defconfig, Python identity, and compiler identity separately.
