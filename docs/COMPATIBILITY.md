...
- **L1 插件（`config-resolve`）**：默认使用当前源码官方 Kconfig/Defconfig 求解 Final Root 组合；开启 A/B 后先在已安装 Feeds 的当前 Source/Branch 元数据上做插件 Root 预检，再求解排除所选插件的 Baseline B，最后求解 Final A，逐环境保留一条配对证据，只证明配置能否成立，不执行软件包编译。若直接 Root 不在该源码/分支，预检记为 `skipped/root-absent-source`，B 与 A 均为 `not-run`，不会进入 Target 或编译阶段。
...
运行状态与兼容性结论是两个轴：`target-prerequisite-failure`、`package-compile-prerequisite-failure`、`rootfs-*-prerequisite-failure` 与 `firmware-prerequisite-failure` 在取得强确定性的上游前置失败证据时，允许的通用 cause 只有 `patch-apply`、`toolchain-kernel-version`、`kernel-prerequisite` 与 `target-build`；它仍然报告为 `inconclusive`（reported inconclusive），但允许该环境 Job 成功结束。证据必须同时保留最内层 failed target 与确定性 error summary，例如 `package/kernel/linux`、`kernel-prerequisite`、`module .../crypto/geniv.ko is missing`；这类前置失败不计入插件主因率。A/B 的 Baseline B 包装失败只有在其内层属于上述 allowlist 时才可绿色终止。它只能通过显式 allowlist 取得该退出状态，不能因此变成 `compatible`、`incompatible` 或 `skipped`。网络、下载、磁盘、OOM/Killed、Runner、timeout、metadata/evidence 缺失，以及没有确定证据的 unattributed failure，都是 operational/unattributed inconclusive，必须保持红色。最终兼容性结论只读取 normalized evidence，不以 Job 的绿色或红色替代证据结论。
