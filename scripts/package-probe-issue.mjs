...
export function probeRunMarkers(comments) {
  ...
  for (const comment of comments || []) {
    ...
    for (const match of body.matchAll(RUN_MARKER_RE)) rows.push({ runId: Number(match[1]), sha256: match[2] });
  }
...
function apiClient(env) {
  ...
  return async (path, options = {}) => {
    ...
    if (!response.ok) {
      const error = new Error(`${options.method || 'GET'} ${path}: HTTP ${response.status}${data?.message ? ` ${data.message}` : ''}`);
...
async function permissionFor(api, owner, repo, actor) {
  ...
  try {
    ...
    if (/HTTP 404\b/.test(error.message)) return 'read';
    throw error;
  }
}
...
async function comment(api, owner, repo, number, body) {
  return await api(`/repos/${owner}/${repo}/issues/${number}/comments`, { method: 'POST', body: { body } });
}
...
async function intake(env, event) {
  ...
  const [currentIssue, beforeDispatch] = await Promise.all([
    ...
    issueComments(api, owner, repo, issue.number),
  ]);
