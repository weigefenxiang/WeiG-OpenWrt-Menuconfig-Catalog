...
assert.equal(l2.calls.filter((args) => args.includes('prepare-tmpinfo')).length, 1);
...
assert.equal(l4.calls.length, 1, 'L4 Final-only must issue one full firmware Make call');
assert.equal(l4.calls[0].length, 1, 'L4 must not execute a hidden Baseline target');
...
assert.equal(l7.calls.length, 1, 'L7 must reuse the one Final firmware build');
