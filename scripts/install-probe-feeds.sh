...
network_failure() {
  local file="$1"
  grep -Eqi \
    'RPC failed|HTTP (429|5[0-9]{2})|returned error: (429|5[0-9]{2})|expected .?packfile|early EOF|Connection (timed out|reset|refused)|Failed to connect|Couldn.t connect to server|Network is unreachable|No route to host|Recv failure|Send failure|Could not resolve host|TLS.*(error|failed)|GnuTLS.*error|SSL.*(error|failed)' \
    "$file"
}
...
fail_stage() {
  ...
  echo "ERROR: Probe feeds failed: reason=${reason} feed=${feed} total_attempts=${attempts} provider=${provider}." | tee -a "$PROBE_LOG"
  exit 1
}
  ...
  for ((provider_index = 0; provider_index < provider_count; provider_index++)); do
    ...
    for ((attempt = 1; attempt <= MAX_ATTEMPTS; attempt++)); do
      ...
      if ((attempt < MAX_ATTEMPTS)); then
        delay=$((attempt * 10))
        echo "Probe feeds: retrying ${feed} on ${provider} in ${delay}s."
        sleep "$delay"
      fi
    done
